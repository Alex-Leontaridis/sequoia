// ChatGPT Message Compressor - Browser Script
...
(function() {
    ...
    // Compress text using the compression service
    ...
    async function checkService() {
        try {
            ...
            if (response.ok) {
                ...
                log('Compression service is running ✓');
                log('Library available:', data.library_available);
            } else {
                error('Compression service returned error status');
            }
        } catch (err) {
            error('Compression service not accessible - make sure it\'s running on localhost:8002');
        }
    }
    
    // Initialize
    log('ChatGPT Message Compressor loaded');
